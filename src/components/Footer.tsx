export function Footer({ onOpenModPanel }: { onOpenModPanel: () => void }) {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="font-serif bg-[#111827] text-white relative">
      {/* Checkered top border */}
      <div 
        className="h-6 w-full"
        style={{
          backgroundImage: `
            linear-gradient(45deg, #F4F6F9 25%, transparent 25%, transparent 75%, #F4F6F9 75%, #F4F6F9),
            linear-gradient(45deg, #F4F6F9 25%, transparent 25%, transparent 75%, #F4F6F9 75%, #F4F6F9)
          `,
          backgroundPosition: '0 0, 12px 12px',
          backgroundSize: '24px 24px',
          backgroundColor: '#111827'
        }}
      />

      <div className="max-w-7xl mx-auto px-6 lg:px-8 pt-12 pb-8">
        
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 border-b border-white/10 pb-12 gap-8 md:gap-0">
          
          {/* Left: Logo & Info */}
          <div className="flex items-center gap-4">
            <button 
              onClick={onOpenModPanel} 
              className="w-16 h-16 rounded-full overflow-hidden shrink-0 border-2 border-[#b58b4c] focus:outline-none hover:scale-105 transition-transform"
              aria-label="Moderator Panel"
            >
              <img 
                src="https://i.imgur.com/jRnV52s.jpg" 
                alt="Mokum Local Kitchen" 
                className="w-full h-full object-cover" 
                referrerPolicy="no-referrer"
              />
            </button>
            <div className="text-left">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                <span className="text-[#3b82f6] font-medium font-sans">Mokum Local Kitchen</span>
                <span className="text-white/40">|</span>
                <span className="text-white/80 font-sans text-sm">KVK 99852667</span>
              </div>
              <p className="text-white/40 text-xs font-sans">&copy; {currentYear} all rights reserved</p>
            </div>
          </div>

          {/* Right: Social Icons */}
          <div className="flex items-center gap-6 text-white/80">
            {/* Facebook */}
            <a href="#" className="hover:text-white transition-colors" aria-label="Facebook">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
              </svg>
            </a>
            {/* TikTok */}
            <a href="#" className="hover:text-white transition-colors" aria-label="TikTok">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5" />
              </svg>
            </a>
            {/* Instagram */}
            <a href="#" className="hover:text-white transition-colors" aria-label="Instagram">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <rect x="2" y="2" width="20" height="20" rx="5" ry="5"/>
                <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/>
                <line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/>
              </svg>
            </a>
            {/* YouTube */}
            <a href="#" className="hover:text-white transition-colors" aria-label="YouTube">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33A2.78 2.78 0 0 0 3.4 19c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.25 29 29 0 0 0-.46-5.33z"/>
                <polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02"/>
              </svg>
            </a>
          </div>

        </div>

        {/* Bottom links */}
        <div className="flex justify-center items-center gap-6 flex-wrap text-sm text-white/60 font-sans">
          <a href="#" className="hover:text-white transition-colors">Zakelijk</a>
          <a href="#" className="hover:text-white transition-colors">Rondvaart</a>
          <a href="#" className="hover:text-white transition-colors">Verhuur</a>
          <a href="#" className="hover:text-white transition-colors">Vacatures</a>
        </div>

      </div>
    </footer>
  );
}
