...
function fixFile(file) {
  ...
  // Let's completely wipe everything from the map start to the map end and put a clean string in.
  const regex = /(\{category\.items\.map\(\(item\) => \{)[\s\S]*?(const product = item\.name;)[\s\S]*?(const prodSelections = selections\[product\] \|\| \{\};)[\s\S]*?(\n\s*\}\)\}\n\s*<\/div>)/;
