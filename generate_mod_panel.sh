...
import { LogIn, X, Clock, Lock, Store, Users, DollarSign, Building2, CheckCircle2, ChevronRight, ArrowLeft } from 'lucide-react';
            ...
            <div className="flex flex-1 overflow-hidden">
              {!isAuthenticated ? (
                ...
                <div className="flex w-full h-full">
                  ...
                  {/* Sidebar Navigation */}
                  <div className="w-64 bg-gray-50 border-r border-gray-200 p-4 shrink-0 overflow-y-auto">
                    <nav className="space-y-2">
                      <button 
                        onClick={() => { setActiveTab('store'); setImpersonating(null); }}
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'store' && !impersonating ? 'bg-[#151f33] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                      >
                        <Store size={18} /> Winkel Status
                      </button>
                      <button 
                        onClick={() => { setActiveTab('registrations'); setImpersonating(null); }}
                        className={`w-full flex items-center justify-between px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'registrations' && !impersonating ? 'bg-[#151f33] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                      >
                      ...
                      <button 
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'customers' || impersonating ? 'bg-[#151f33] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                      >
                        <Users size={18} /> Klanten (Kantoren)
                      </button>
                      <button 
                        onClick={() => { setActiveTab('prices'); setImpersonating(null); }}
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'prices' && !impersonating ? 'bg-[#151f33] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
                      >
                            ...
                            {registrations.map(reg => (
                              <div key={reg.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <button 
                                  ...
                                  className="shrink-0 bg-green-600 text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-green-700 transition-colors flex items-center gap-2"
                                >
                                  ...
                                  <td className="px-6 py-4 text-right">
                                    <div className="relative inline-block w-32">
                                      ...
                                      <input 
                                        ...
                                        className="w-full pl-8 pr-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-[#151f33] focus:ring-1 focus:ring-[#151f33]"
                                      />
                                    </div>
                                  </td>
                                </tr>
